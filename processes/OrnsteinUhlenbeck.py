...
class OrnsteinUhlenbeck:
    def __init__(self,mean=0,sigma=1,theta=1,r_0=0,t_0=0, t_n=1, n_steps=1000):
        ...
        if not(np.shape(self.sigma)[0] == np.shape(self.sigma)[1] == self.dim == np.shape(self.theta)[0] == np.shape(self.theta)[1]):
    ...
    def simulate(self, method="euler-maruyama"):
        if method == "euler-maruyama":
            self.path = EulerMaruyama(self.drift,self.diffusion,self.r_0,self.t_0,self.t_n,self.n_steps,self.dim).solve()[0,:,:]
        if method == "exact":
            if self.dim > 1:
                raise ValueError("The exact method is only implemented for 1D processes.")
            self.path = np.zeros((self.n_steps+1, 1))
            self.path[0] = self.r_0
            Z = pyrandom.crandom.normal(0, 1, self.n_steps)
            for i in range(1,self.n_steps+1):
                self.path[i] = (self.mean+ (self.path[i-1] - self.mean) * np.exp(-self.theta * self.dt) + self.sigma * np.sqrt((1 - np.exp(-2 * self.theta * self.dt)) / (2 * self.theta)) * Z[i-1])
        return self.path
