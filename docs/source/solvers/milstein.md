...
## Import line
You can import the Milstein class from the `sde` module as follows:
```python
...
```python
pystochastic.sde.Milstein(drift = lambda x,t : 2*x, diffusion = lambda x,t : 1, initial = 0.2, T = 1, steps = 1000)
```
...
The Milstein scheme is a way to solve an autonomous and unidimensional stochastic differential equation of the following form:
...
To proceed, the Milstein scheme introduces a partition of the time interval `[0,T]` into `steps+1` time steps $0=t_0 < t_1 < \dots < t_{\text{steps}} = T$.
...
\begin{equation*}
    \left\{ \begin{array}{l}
        \tilde{X}_0 = X_0 \\
        \tilde{X}_{t_{i+1}} = \tilde{X}_{t_i} + \mu(\tilde{X}_{t_i}) \Delta t + \sigma(\tilde{X}_{t_i}) \Delta W_{t_i} + \frac{1}{2} \sigma(\tilde{X}_{t_i}) \sigma'(\tilde{X}_{t_i}) \left((\Delta W_{t_i})^2 - \Delta t\right)
    \end{array} \right.
\end{equation*}
...
> [!NOTE]
> The Milstein method is used to numerically simulate unidimensional autonomous diffusion processes implemented by the [Diffusion Process](<project:/processes/diffusion/index.md>) class.
...
`diffusion_derivative` : _function_ or `None`
: Derivative of the diffusion function $\sigma$. If None, the derivative is computed numerically, with a numerical tolerance of 1e-6. The approximation might be unstable for
very smooth diffusion functions.
...
> [!NOTE]
> The NumPy vectorization always works for the unidimensional Milstein solver.
...
\begin{equation*}
dX_t = \frac{X_t}{2} dt - X_t dW_t
\end{equation*}

on the interval $[0,1]$, with the initial value $X_0 = 1$.
```python
import numpy as np
from pystochastic.sde import Milstein

solver = Milstein(drift = lambda x : x/2, diffusion = lambda x : -x, initial = 1, T = 1, steps = 1000)
